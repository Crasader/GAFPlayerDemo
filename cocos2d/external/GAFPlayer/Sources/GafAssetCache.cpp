...
namespace gaf
{
    ...
    {
        static GafAssetCache gafAssetCache;
        return &gafAssetCache;
    }

    std::string GafAssetCache::getGafMemoryInfo() {
        std::string info = "";
        char momery[128];
        double totalMomery = 0.0;
        auto i = 0;
        for (auto it = _gafAssets.cbegin(); it != _gafAssets.cend(); ++it, ++i) {
            sprintf(momery, "%d KB",
                    it->second->getTextureManager()->getMemoryConsumptionStat()/1024);
            info += "path:" + it->first + " => " + momery + "\n";
            totalMomery +=
                it->second->getTextureManager()->getMemoryConsumptionStat()*1.0f/1024.0f;
        }
        
        sprintf(momery, "%d gafs, for %d KB (%f MB)", i, (int)totalMomery, totalMomery/1024.0f);
        info += std::string("GafAssetCache getGafMemoryInfo:  ") + momery + "\n";
        return info;
    }
